...
# Quiz: make the function query() return a list of the ID's of the links that were
# submitted by the user 62443 sorted by submission time ascending
def query():
	cursor = db.execute("select id from links where submitter_id=62443 order by submitted_time asc")
	results = [t[0] for t in cursor] # for tuple in cursor, the 1st column is always going to be 'id'
	print results
